...
uxTour.prototype.showStep = function () {
    ...
    var step = this.tour.steps[this.currentStep], browser = this.getPrefix(),
        prefix = (browser.dom === 'Moz' ? browser.js : browser.lowercase),
        documentBody = (browser.lowercase === 'webkit' ? document.body : document.documentElement), padding, element,
        rect, marginTop, marginLeft;
...
uxTour.prototype.setText = function (step) {
    ...
    var element = document.getElementById(step.element), elementRect = element.getBoundingClientRect(),
        tooltipRect = this.tooltip.getBoundingClientRect(), direction = step.direction || 'bottom',
...
uxTour.prototype.scroll = function (element, to, duration) {
    ...
    var start = element.scrollTop, change = to - start, increment = 20, that = this, position;
